...
contract AxiomV2QueryMock is IAxiomV2Query, AxiomAccess, UUPSUpgradeable {
  ...
  function fulfillQuery(
    ...
    if (
      queryWitness.callbackHash != keccak256(abi.encodePacked(callback.target, callback.extraData))
    ) {
  ...
  function _computeQueryId(
    ...
    queryId = uint256(
      keccak256(
        abi.encodePacked(uint64(block.chainid), caller, userSalt, queryHash, callbackHash, refundee)
      )
    );
  }
  ...
  function _verifyAndWriteResult(
    ...
    bytes32 resultHash = keccak256(
      abi.encodePacked(sourceChainId, dataResultsRoot, dataResultsPoseidonRoot, computeResultsHash)
    );
