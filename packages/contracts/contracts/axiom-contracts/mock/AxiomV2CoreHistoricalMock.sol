...
contract AxiomV2CoreHistoricalMock is IAxiomV2Core, AxiomAccess, UUPSUpgradeable {
  ...
  function appendHistoricalPMMR(
    ...
    bytes32 commitment = keccak256(
      abi.encodePacked(prevHashes[roots.length - 1], roots[roots.length - 1], lastNumFinal)
    );
    if (historicalRoots[startBlockNumber] != commitment) {
      revert AxiomBlockVerificationFailed();
    }
