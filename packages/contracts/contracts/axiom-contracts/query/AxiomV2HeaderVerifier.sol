...
contract AxiomV2HeaderVerifier is IAxiomV2HeaderVerifier, AxiomAccess, UUPSUpgradeable {
  ...
  function verifyQueryHeaders(
    ...
    if (proofMmrSize <= mmrWitness.snapshotPmmrSize) {
      ...
      MerkleMountainRange.MMR memory proofBatchMmr = MerkleMountainRange.fromPeaks(
        mmrWitness.proofMmrPeaks,
        0,
        BLOCK_BATCH_DEPTH
      );
      ...
      if (
        mmrWitness.snapshotPmmrSize - (mmrWitness.snapshotPmmrSize % BLOCK_BATCH_SIZE) >=
        proofMmrSize
      ) {
      ...
      if (
        completePmmrKeccak !=
        IAxiomV2State(axiomCoreAddress).pmmrSnapshots(mmrWitness.snapshotPmmrSize)
      ) {
      ...
      if (
        ...
        bytes32 snapshotLeaf = MerkleMountainRange
          .fromPeaks(mmrWitness.mmrComplementOrPeaks, 0, BLOCK_BATCH_DEPTH)
        ...
        MerkleMountainRange.MMR memory snapshotMmr = MerkleMountainRange.fromPeaks(
          mmrWitness.mmrComplementOrPeaks
        );
        ...
        MerkleMountainRange.MMR memory proofMmr = MerkleMountainRange.fromPeaks(
          mmrWitness.proofMmrPeaks
        );
        proofMmr.appendLeaves(append);

        MerkleMountainRange.MMR memory corePmmrPeaks = IAxiomV2State(axiomCoreAddress)
          .blockhashPmmrPeaks();
